...
if __name__ == "__main__":
    ...
    for agent_name in agent_names:
        ...
        actors[agent_name] = MessageActor(action_size, len(red_agents), args.n_hidden).to(device)
        critics[agent_name] = MessageCritic(action_size, args.n_hidden, 5*len(red_agents)).to(device)
        ...
        for epoch in range(args.update_epochs):
            ...
            for agent_name in red_agents:
                for start in range(0, args.batch_size, args.minibatch_size):
                    ...
                    _, _, newlogprob, entropy = actors[agent_name].get_action(
                    ...
                    newvalue = critics[agent_name].get_value(torch.stack(all_buffer_obs),
                                                             buffers[agent_name]["recv_msg"][mb_inds]).squeeze(-1)
