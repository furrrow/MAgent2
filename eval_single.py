...
class Args:
    ...
    save_freq: int = 10
    checkpoint_path: str = "/home/jim/Documents/Projects/MAgent2/saves/naive_single__size25__5__2025-05-11_17-59-28/checkpoint_150.pt"  # Model load file name, "" doesn't load
    config_file: str = "/home/jim/Documents/Projects/MAgent2/saves/naive_single__size25__5__2025-05-11_17-59-28/config.yaml"  # Model load file name, "" doesn't load

    # Algorithm specific arguments
    env_id: str = "eval_single"
...
def get_action_value_map(agent, observation):
    ...
    for i in range(h):
        ...
        for j in range(w):
            ...
            map_element[:, map_row_0: map_row_1, map_col_0: map_col_1] = observation[:, obs_row_0:obs_row_1, obs_col_0:obs_col_1]
            map_idx.append(idx)
            x_list.append(j)
            y_list.append(i)
    ...
    return np.array(x_list), np.array(y_list), np.array(arrows), value_map.cpu().detach().numpy()
...
if __name__ == "__main__":
    ...
    for k in range(n_eval):
        ...
        for agent_name in env.agent_iter():
            ...
            xs, ys, arrows, value_map = get_action_value_map(agents[agent_name], obs_agent)
            # plot the value map
            value_map = value_map.reshape(13, 13).T

            # now to plot this action map.....
            # Creating plot
            fig2, ax2 = plt.subplots(figsize=(4, 4))
            im = ax2.imshow(value_map)
            ...
            fig2.canvas.draw()
            plt.savefig(f'eval/eval{k}_arrow{step}.png')
        ...
        if args.capture_video:
            if len(frame_list) > 0:
                frame_list[0].save(f'eval/{args.exp_name}_{env_name}_eval.gif', save_all=True, append_images=frame_list[1:],
                                       duration=3, loop=0)
